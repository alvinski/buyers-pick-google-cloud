...
if(isset($_REQUEST['email']) && $_REQUEST['email']!="")
{
	...
	if($num_row>0)
	{
		...
		$sqlAttr = mysql_query("select * from ba_tbl_attributes where user_id = '$user_id' and is_deleted = 1");
		...
		$sqlVendorMaster = mysql_query("select * from ba_tbl_vendor_master where user_id = '$user_id' and is_deleted = 1");
		...
		foreach($content_master_id as $key_con_master=>$con_master_id)
		{
			...
			$sqlAttrContLink = mysql_query("select * from ba_tbl_attributes_content_link where content_id = '$c_mas_id' and is_deleted = 1'");
